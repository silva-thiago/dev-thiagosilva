...
const Navbar: React.FC<IProps> = ({ siteTitle }) => {
      ...
      <nav className='navbar'>
        {/* <div className='d-flex justify-content-between items-center mb-0'>
          <Link className='navbar-brand' to='/'>
            <Img className='logo' fluid={data.file.childImageSharp.fluid} alt='Logotipo' loading='auto' />
            <span className='logo-text'>{siteTitle} Developer_</span>
          </Link>
        </div> */}
        <ul className='navbar-nav'>
          <li className='logo'>
            <a href='#' className='nav-link'>
              <span className='logo-text'>{`TS{Dev}_`}</span>
              <svg
                aria-hidden='true'
                focusable='false'
                data-prefix='fad'
        ...
        {/* {links.map(({ href, label, target }) => (
          <a className='nav-link' key={`${href}${label}${target}`} href={href} target={target} rel='noopener noreferrer' role='button'>
            <img src='images/icons/apple.svg' alt='Icon'/>
            <span>{label}</span>
          </a>
        ))} */}
      </nav>
